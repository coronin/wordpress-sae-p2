...
post http://wpjam.net/api/ with body: a:3:{s:6:"action";s:6:"domain";s:2:"id";i:56;s:6:"domain";s:22:"tsg2011.vipsinaapp.com";}
GET:a:1:{s:4:"page";s:12:"weixin-robot";}

 */

if(!function_exists('wpjam_net_api_request')){
	function wpjam_net_api_request( $args ) {
        echo '<div class="error" style="color:red;"><p>post http://wpjam.net/api/ with body: '; // LC
        print_r( serialize($args) ); // LC
        echo '<br/>GET:'; // LC

        $request = wp_remote_post( 'http://wpjam.net/api/', array( 'body' => $args, 'timeout'=>3 ) );

        print_r( serialize($_GET) ); // LC
        echo'</p></div>'; // LC
...
function wpjam_net_get_domain(){
    ...
    //	return '';
    //}
    return 'tsg2011.vipsinaapp.com'; // LC
}
