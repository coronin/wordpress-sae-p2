...
if(!function_exists('wpjam_net_api_request')){
    function wpjam_net_api_request( $args ) {
        echo '<div class="error" style="color:red;"><p>request:';
        print_r($args);
        echo '<br/>GET:';
        print_r($_GET);
        echo'</p></div>';
...
function wpjam_net_check_domain($id = 56){
    $domain = wpjam_net_get_domain('wpjam_net_check_domain with id '.$id.' (default 56)');
...
function wpjam_net_get_domain($in){
    sae_set_display_errors(false);
    sae_debug('weixin-robot > wpjam_net_get_domain: call from '.$in);
    return '';
}
